...
function App() {
    ...
    <main>      
      ...
      <Door 
       ...
       <Door 
        ...
        number='25' 
        date='2022-12-25'
        challenge="Have a merry Christmas and enjoy your day with family, friends and with lots of laughter"
        joke=""
        jokeAnswer=""
        modalShow={modalShow} 
        setModalShow={setModalShow} 
  ...
  function Door({ number, date, challenge, joke, jokeAnswer, modalShow, setModalShow, setDay, setJoke, setChallenge }) {    
    ...
    return (
    <>  
      <article className={isEnabled ? "present" : "present disabled"} onClick={() => { 
            if (isEnabled){
              setDay(number)    
        ...
        <div className={isEnabled ? "present__pane" : "present__pane_disabled disabled"}>
          <h2 className={isEnabled ? "present__date" : "present__date"}>{number}</h2>
        </div>
        <div className="present__content"> 
          <p className="present__inside">
            { isEnabled ? "Click for activity" : "" }
          </p>
        </div> 
      </article>
    </> 
